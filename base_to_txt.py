...
def should_ignore(path, root_pathspec, additional_pathspecs):
    ...
    rel_path = os.path.relpath(
        path, start=os.path.dirname(os.path.abspath(__file__)) + "/.."
    )
    ...
    ignore_patterns = [
        ...
        ".idea",
        "*.html",
        "codebase_documentation.md",  # Ignore the output file itself
    ]
...
def generate_directory_tree(start_path, root_pathspec, additional_pathspecs, indent=""):
    ...
    for item in items:
        ...
        rel_path = os.path.relpath(
            item_path, start=os.path.dirname(os.path.abspath(__file__)) + "/.."
        )
        ...
        if os.path.isdir(item_path):
            ...
            subtree = generate_directory_tree(
                item_path, root_pathspec, additional_pathspecs, indent + "  "
            )
            tree.extend(subtree)
        else:
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for root, dirs, files in os.walk(root_dir):
            ...
            for file in sorted(files):
                ...
                if rel_path in processed_files or should_ignore(
                    file_path, root_pathspec, additional_pathspecs
                ):
                    continue
                ...
                try:
                    # For CSV files, only read the first 10 lines
...
def main():
    ...
    root_dir = os.path.join(script_dir)
    output_file = os.path.join(root_dir, "codebase_documentation.md")
